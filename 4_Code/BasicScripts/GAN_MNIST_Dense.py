...
GAN - with Fully Connected Layers and no CNNs using Keras+TensorFlow
...
BATCH_SIZE = 128
EPOCHS = 50
noise_dim = 100 # 100-Deep, 392-Shallow
...
train_dataset = tf.data.Dataset.from_tensor_slices(train_images).shuffle(BUFFER_SIZE).batch(BATCH_SIZE) # [0:30000,:,:,:]

#train_dataset = tf.data.Dataset.from_tensor_slices(train_images[0:30000,:,:,:]).shuffle(BUFFER_SIZE).batch(BATCH_SIZE) # [0:30000,:,:,:]
...
def make_generator_model():
    ...
    # Deep + Shallow
    model.add(layers.Dense(100, input_shape=(noise_dim,)))
    ...
    # Deep + Shallow
    model.add(layers.Dense(300, input_shape=(noise_dim,)))
    model.add(layers.BatchNormalization())
    model.add(layers.LeakyReLU())
    ...
    model.add(layers.Dense(784))  # activation = 'tanh in shallow    
...
def make_discriminator_model():
    ...
    # Deep + Shallow
    model.add(layers.Dense(600, input_shape=(noise_dim,)))
    model.add(layers.BatchNormalization())
    model.add(layers.LeakyReLU())   
    ...
    # Deep 
    model.add(layers.Dense(400, input_shape=(noise_dim,)))
    model.add(layers.BatchNormalization())
    model.add(layers.LeakyReLU())
    ...
    # Deep + Shallow
    model.add(layers.Dense(100)) # activation = 'sigmoid' in shallow
    model.add(layers.BatchNormalization())
    model.add(layers.LeakyReLU())     
...
# Generator Optimizer
generator_optimizer = tf.keras.optimizers.Adam(1e-6) # 1e-5

# Discriminator Optimizer
discriminator_optimizer = tf.keras.optimizers.Adam(1e-6) # 1e-5
